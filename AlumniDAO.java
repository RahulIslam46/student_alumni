...
public class AlumniDAO {
  ...
  public static void ensureProfilePictureColumn() {
    String query =
        "ALTER TABLE profiles ADD COLUMN IF NOT EXISTS profile_picture VARCHAR(500) DEFAULT NULL";
  ...
  public static boolean sendMessage(int senderId, int receiverId,
    ...
    if (!userExists(senderId) || !userExists(receiverId)) {
      ...
      System.err.println("Sender ID " + senderId +
                         " exists: " + userExists(senderId));
      System.err.println("Receiver ID " + receiverId +
                         " exists: " + userExists(receiverId));
      return false;
    }
    ...
    try (Connection conn = DatabaseConnection.getConnection();
      ...
      System.out.println("✅ Message sent successfully! Rows affected: " +
                         rowsAffected);
  ...
  public static boolean createNotification(Notification notif) {
    ...
    try (Connection conn = DatabaseConnection.getConnection();
      ...
      if (notif.getApplicationDeadline() != null &&
          !notif.getApplicationDeadline().trim().isEmpty()) {
        try {
          ...
          System.err.println(
              "⚠️ Invalid date format: " + notif.getApplicationDeadline() +
              ", setting to NULL");
